...
public class DbUpdateOk extends HttpServlet{
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int age = (request.getParameter("age")==null || request.getParameter("name").equals("")) ? 0 : Integer.parseInt(request.getParameter("age"));
		...
		if(res != 0) {
			request.setAttribute("message", "회원 정보수정이 완료 되었습니다");
			request.setAttribute("url", "/study2/database/DbList");
		} 
		else {
			request.setAttribute("message", "회원 정보수정 실패 되었습니다");
