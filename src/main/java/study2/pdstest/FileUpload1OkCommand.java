...
public class FileUpload1OkCommand implements StudyInterface {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		/*																															
		  ...
		  사용법 : MultipartRequest(저장소(request),서버의 저장될 파일의 경로,서버의 저장될 파일의 최대용량,코드변환방식,기타옵션(파일명중복방지클래스)) 
		...
		*/
		...
		String encoading = "UTF-8";
		
		// 파일 업로드 처리.....(객체 생성과 동시에 자동 업로드)
		MultipartRequest multipartRequest = new MultipartRequest(request, realPath,maxSize,encoading, new DefaultFileRenamePolicy()); 
