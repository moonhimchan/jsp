...
<div class="container">
  ...
  <table class="table table-borderless m-0 p-0">
  	<tr>
  		<td class="text-left">
  			<!-- <input type="button" values="글올리기" onclick="location.href=''" class="btn btn-success" /> -->
				<a href="GuestInput.gu" class="btn btn-success">글올리기</a>  		<!-- 확장자 패턴은 주소를 다 안써도 된다. -->
  		</td>
  		<td class="text-right">
  			<c:if test="${sAdmin != 'adminOk'}"><a href="GuestAdmin.gu" class="btn btn-primary">관리자</a></c:if>		
  			<c:if test="${sAdmin == 'adminOk'}"><a href="GuestAdminOut.gu" class="btn btn-primary">관리자로그아웃</a></c:if>		
  		</td>
  	</tr>
  </table>
  ...
  <table class="table table-borderless">
  	<tr>
  		<td class="text-left">
  			번호 : ${guestNo}
  			<!--<c:if test="${sAdmin=='adminOk'}"><a href="javacript:deleteCheck(${vo.idx})" class="btn btn-danger btn-sm">삭제</a></c:if> javacript 스펠링 오류-->
